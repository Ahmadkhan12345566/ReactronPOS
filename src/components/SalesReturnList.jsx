...
export default function SalesReturnList({ salesReturns, setShowForm }) {
  ...
  // Status options
  const statusOptions = useMemo(() => ['All', 'Received', 'Pending'], []);
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    const headers = [['#', 'Product', 'Date', 'Customer', 'Status', 'Total', 'Paid', 'Due', 'Payment Status']];
    const data = filteredData.map((item, index) => [
      index + 1,
      item.product.name,
      item.date,
    ...
    doc.save(`sales_returns_${new Date().toISOString().slice(0,10)}.pdf`);
  };
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600">Manage your returns</p>
          </div>
