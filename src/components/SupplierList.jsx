...
export default function SupplierList({ Suppliers, setShowForm }) {
  ...
  const exportToExcel = () => {
    const data = filteredData.map((Supplier, index) => ({
      '#': index + 1,
      Code: Supplier.code,
  ...
  const exportToPDF = () => {
    const doc = new jsPDF();
    const headers = [['#', 'Code', 'Supplier', 'Email', 'Phone', 'address', 'products', 'Status']];
    const data = filteredData.map((Supplier, index) => [
      index + 1,
      Supplier.code,
      Supplier.name,
  ...
  const filteredData = useMemo(() => {
    return Suppliers.filter(Supplier => 
      ...
      `${Supplier.name} ${Supplier.email} ${Supplier.phone} ${Supplier.address} ${Supplier.products}`.toLowerCase().includes(search.toLowerCase())
    );
