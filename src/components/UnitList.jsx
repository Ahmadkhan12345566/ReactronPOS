...
export default function UnitList({ units, setShowForm }) {
  ...
  const exportToExcel = () => {
    const data = filteredData.map(unit => ({
      '#': filteredData.indexOf(unit) + 1,
      Unit: unit.name,
      'Short Name': unit.shortName,
      'No of Products': unit.productCount,
      'Created Date': unit.createdOn,
      Status: unit.status
    }));
  ...
  const filteredData = useMemo(() => {
    return units.filter(unit => 
      ...
      `${unit.name} ${unit.shortName}`.toLowerCase().includes(search.toLowerCase())
    );
      ...
      <div className="bg-white rounded-xl p-4 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-600">Manage your units</p>
          </div>
