...
export default function BrandList({ brands, setShowForm }) {
  ...
  const exportToExcel = () => {
    const data = filteredData.map(brand => ({
      '#': filteredData.indexOf(brand) + 1,
      Brand: brand.name,
      'Created Date': brand.createdOn,
      Status: brand.status
    }));
    ...
    XLSX.writeFile(workbook, `brands_${new Date().toISOString().slice(0,10)}.xlsx`);
  };
