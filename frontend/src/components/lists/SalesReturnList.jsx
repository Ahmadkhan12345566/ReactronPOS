...
import { selectColumn, indexColumn, actionsColumn } from '../ListComponents/columnHelpers';
...
export default function SalesReturnList({ salesReturns, setShowForm }) {
  ...
  const customerOptions = useMemo(() => {
    const customers = [...new Set(salesReturns.map(item => item.customer?.name).filter(Boolean))];
          ...
          <div className="flex items-center">
            <div className="bg-gray-100 rounded-full p-1 mr-3">
              <img 
                ...
                alt={row.original.customer?.name || 'Customer'} 
                className="w-8 h-8 rounded-full object-cover"
              />
            </div>
            <span>{row.original.customer?.name || 'Unknown Customer'}</span>
          </div>
        ),
        size: 200,
      },
